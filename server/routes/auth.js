...
router.post("/login", async (req,res)=>{
  try{
    ...
    if(user.status !== "approved"){
      return res.json({msg:"Wait for approval"});
    }
...
router.get("/admin/stats", async (req,res)=>{
  ...
  const approvedColleges = await User.countDocuments({role:"college",status:"approved"});
  const pendingColleges = await User.countDocuments({role:"college",status:"pending"});

  const totalIndustry = await User.countDocuments({role:"industry"});
  const approvedIndustry = await User.countDocuments({role:"industry",status:"approved"});
  const pendingIndustry = await User.countDocuments({role:"industry",status:"pending"});

  res.json({
    students:{total:totalStudents,approved:approvedStudents,pending:pendingStudents},
    colleges:{total:totalColleges,approved:approvedColleges,pending:pendingColleges},
    industry:{total:totalIndustry,approved:approvedIndustry,pending:pendingIndustry}
  });
});
